      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="flex flex-wrap gap-4 mb-12 border-b border-slate-200 dark:border-white/10 pb-6 sticky top-24 z-30 bg-white/80 dark:bg-[#030303]/80 backdrop-blur-xl transition-colors duration-500">
          {categories.map((cat) => (
            <button
              ...
              className={`px-6 py-2 rounded-full text-sm font-mono uppercase tracking-wider transition-all duration-300 border ${filter === cat
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-16">
          {filteredProjects.map((project, index) => (
            ...
            >
              <div className="aspect-square overflow-hidden rounded-xl bg-slate-200 dark:bg-zinc-900 mb-6 relative border border-slate-200 dark:border-white/5 shadow-lg dark:shadow-none">
                <img
                  src={project.bannerUrl || project.imageUrl}
                  alt={project.title}
                  className="relative z-10 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105 opacity-95 group-hover:opacity-100"
                />
                <div className="absolute inset-0 bg-black/20 dark:bg-black/40 group-hover:bg-transparent transition-colors duration-500 z-20 pointer-events-none"></div>
